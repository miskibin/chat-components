...
| **todo-list** | The agent's plan as one line above the composer — live task, count, checklist on click; `parseTodoItems` reads a todo tool call's arguments | [todo-list](https://chat-input-azure.vercel.app/docs/components/todo-list) |
...
| **message-parts** | `MessageProcess`, `MessageReasoning`, `MessageToolCall(s)`, `MessageCode`, `MessageArtifact`; Ask Question tools render `AskQuestion`, todo tools render `TodoList` | [message-parts](https://chat-input-azure.vercel.app/docs/components/message-parts) |
